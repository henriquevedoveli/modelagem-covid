...
ui <- navbarPage(theme = shinytheme("spacelab"), collapsible = TRUE,
                          ...
                          sidebarLayout(
                              sidebarPanel(
                                  tabsetPanel(
                                      ...
                                      tabPanel(withMathJax('Obtenção de \\(\\beta\\)'),
                                               br(),
                                               br(),
                          ...
                          sidebarLayout(
                              sidebarPanel(
                                  ...
                                  tabPanel('Sobre os Parâmetros',
                                          ...
                                          br(),
                                          h4('Parâmetros Utilizados no Modelo SEIR'),
                                          p('Para o Modelo', em(strong('SEIR')),'são: '),
                                          ...
                                          p(em(strong('Recuperados (R): ')), 'para o modelo',em(strong('SEIR')), 'o grupo de recuperados é formado pelas pessoas que se recuperaram da
                                            infecção e também por pessoas que venham a falecer. Para o modelo',em(strong('SEIRHM')), ' o grupo dos recuperados é formado pela parcela da
                                            população que se curaram do vírus.'),
                                          ...
                                          h4('Parâmetros Utilizados no Modelo SEIRHM'),
                                          h5('(SEIR Modificado)'),
                                          p(em(strong('Mortos (M): ')), ' '),
                                          p(em(strong('Hospitalizados (H): ')), ' '),
                                          p(withMathJax('\\(\\theta\\): ')),
                                          p(withMathJax('\\(\\eta\\): ')),
                                          p(withMathJax('\\(\\rho\\): '))
                 ...
                 tabPanel('Sobre o Site',
                          tags$div(
                              ...
                              h6('16/07/2020'), # Ultima Atualizacao
    ...
    output$CALCULADORA_seirhm <- renderPlotly({
        ...
        sir_equations <- function(tempo, variaveis, parametros){
            with(as.list(c(variaveis, parametros)), {
                dS = -beta*S*I
                dE = beta*S*I - sigma*E
                dI = sigma*E - (1-theta)*gamma*I - theta*gamma*I
                dR = theta*gamma*I + (1-eta)*rho*H
                dH = (1-theta)*gamma*I - eta*rho*H - (1-eta)*rho*H
                dM = eta*H*rho
        ...
        fig <- fig %>% add_trace(y=~N*seir$R, name="Recuperados", mode= 'lines',
        ...
        fig <- fig %>% layout(title='Modelo SEIR Modificado', hovermode = TRUE, spikedistance = -1,
                              xaxis = list(title='Dias'),
                              yaxis = list(title='Casos'))
...
shinyApp(ui = ui, server = server)
>>>>>>> 8dbd6e96f349b8729bcea679d7910298f13c5eba
